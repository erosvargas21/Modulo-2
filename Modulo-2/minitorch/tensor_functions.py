...
def make_tensor_backend(tensor_ops, is_cuda=False):
    ...
    class Backend:
        ...
        class Mean(Function):
            ...
            def backward(ctx, grad_output):
                # TODO: Implement for Task 2.3.
                a_sh, a_sz, dm = ctx.saved_values
                if dm is None:
                    obt = grad_output.zeros(a_sh)
                    obt._tensor._storage[:] = grad_output[0]/a_sz
                    return obt

                else:
                    return grad_output/a_sh[dm]
        class LT(Function):
            @staticmethod
            def forward(ctx, a, b):
                # TODO: Implement for Task 2.2.
            ...
            def backward(ctx, grad_output):
                # TODO: Implement for Task 2.3.
                a_sh, b_sh = ctx.saved_values
                return zeros(a_sh), zeros(b_sh)

        class EQ(Function):
            @staticmethod
            def forward(ctx, a, b):
                # TODO: Implement for Task 2.2.
                ctx.save_for_backward(a.sh,b.sh)
                return eq_zip(a,b)
