...
def count(position, shape, out_index):
    ...
    # TODO: Implement for Task 2.1.
    loc=position
    for o in range(len(out_index)-1,-1,-1):
        out_index[o] = loc % shape[o]
        loc = loc//shape[o]
...
def broadcast_index(big_index, big_shape, shape, out_index):
    ...
    for k in range(len(shape)):
        if shape[k] > 1:
            out_index[k] = big_index[k + (len(big_shape) - len(shape))]
        else:
            out_index[k] = 0
    return None
...
class TensorData:
    ...
    def permute(self, *order):
        ...
        # TODO: Implement for Task 2.1.
        n_shape = tuple([self.shape[k] for k in order])
        n_strides = tuple([self.strides[k] for k in order])
